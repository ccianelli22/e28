  ...
  <div class="container mx-auto text-center">
    <div class="inline-flex flex-col">
      ...
      <p class="p-1 text-xl" v-if="props.mathOpName !== 'exponents'">
        What is {{ problem.num1 }} {{ props.mathOp }} {{ problem.num2 }}?
      </p>
      ...
      <div>
        ...
        <p
          class="text-red-700"
          v-if="problem.correct === false && problem.submitted === true"
        >
          Good Try!
        </p>
        <p :class="problem.answerStyle" v-if="problem.submitted === true">
...
export default {
  ...
  setup(props) {
    ...
    const updateScore = async () => {
      scoreData.scores[props.mathOpName].lastPracticeDate = new Date();
      scoreData.scores[props.mathOpName].attempts += 1;
      if (problem.correct === true) {
        scoreData.scores[props.mathOpName].correct += 1;
      }
      scoreData.totalCorrectPercent = getTotalPercentage(scoreData.scores);
    ...
    const onSubmit = () => {
      let isValid = true;
      if (problem.userAnswer === "" || isNaN(Number(problem.userAnswer))) {
        isValid = false;
      ...
      if (problem.submitted === false && isValid) {
        ...
        problem.answerStyle = correct === 1 ? "text-green-700" : "text-red-700";
        //SEND DATA TO DB!!!!
        updateScore();
        setTimeout(() => {
          problem.userAnswer = "";
