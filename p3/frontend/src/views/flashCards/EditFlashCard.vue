	...
	export default {
		...
		setup() {
			...
			const deleteFlash = async () => {
				...
				if (confirmation) {
					...
					let success = await axios.delete(
						`/api/flashCards/${store.state.username}/${flashCardValues["_id"]}`,
					)
			...
			const submitFlash = async () => {
				if (
					flashCardValues["hint"].trim() ===
					flashCardValues["answer"].trim()
				) {
					fieldError.isError = true
					fieldError.errorMessage =
						"The hint cannot be the answer!"
				}
				if (flashCardValues["answer"].length < 1) {
					fieldError.isError = true
					fieldError.errorMessage =
						"The Answer Field cannot be blank."
				}
				if (flashCardValues["question"].length < 1) {
					fieldError.isError = true
					fieldError.errorMessage =
						"The Question Field cannot be blank."
				}
