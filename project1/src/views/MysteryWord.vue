...
<div class="flex flex-col">
    <button id="mysteryWord" class="text-sm italic underline font-semibold hover:text-purple-600 duration-300 transition" type="button" @click="modalFunc.openModal">
        SHOW RULES
    </button>
    <div class="flex flex-col m-4" v-if="mysteryWordGame.playingGame">
        <div class="flex-1">
            <img id="vampire" class="rounded relative w-32 h-32 pr-3 duration-300 transition opacity-100" :src="require('../assets/mysteryWordImages/vampire.png')" />
            <img id="user" class="rounded float-right w-64 h-32 pr-3 duration-300 transition opacity-100" :src="require('../assets/mysteryWordImages/sleeping.png')" />
        </div>
        <div class="self-center">
            <span class="mx-2 text-2xl" v-for="(letter, index) in mysteryWordGame.hiddenWord" :key="index">
                {{ letter }}
            </span>
        </div>
        <div class="flex flex-row items-baseline mt-6 self-center" v-if="!mysteryWordGame.winner">
            <input class="border-2 p-2 border-gray-300 border-solid hover:bg-orange-300 duration-300 transition-colors" type="text" maxlength="1" v-model="mysteryWordGame.letterInput" @keyup.enter="submit" />
            <button class="ml-4 p-2 rounded border-2 border-gray-300 border-solid hover:bg-orange-500 duration-300 transition-colors" type=" button" @click="submit">
                Submit
            </button>
        </div>
        <h3 class="text-red-600 mt-2 text-3xl self-center font-semibold" v-if="mysteryWordGame.winner === 'computer'">
            YOU HAVE BEEN BITTEN! THE CORRECT WORD WAS
            {{ mysteryWordGame.wordSelected }}
        </h3>
        <h3 class="text-green-600 mt-2 text-3xl self-center font-semibold" v-if="mysteryWordGame.winner === 'user'">
        ...
        <div class="flex flex-col items-baseline mt-6 self-center">
            ...
            <div class="flex flex-row">
                <span class="text-sm mx-2 text-red-600 font-semibold" v-for="(letters,

index) in mysteryWordGame.lettersTried" :key="index">
                    {{ letters }}
                </span>
            </div>
        </div>
    ...
    <div class="flex flex-col items-center mt-20 justify-center text-lg relative top-0" v-else>
        <p class="font-semibold italic">
            How many letters do you want your word to be?
        </p>
        <div class="flex mt-4 text-center">
            <input class="p-2 mr-4 rounded-full border-2 border-solid border-gray-300 hover:bg-orange-300 duration-300 transition-colors" type="number" min="3" max="12" v-model="mysteryWordGame.numberFilter" />
            <button class="p-2 rounded-full border-2 border-solid border-gray-300 hover:bg-orange-500 duration-300 transition-colors" type="button" @click="startGame">
                Play
            </button>
        </div>
    </div>
    <teleport to="body" v-if="modal.showModal && modal.activeModalID === 'mysteryWord'">
        <Modal header="Rules">
